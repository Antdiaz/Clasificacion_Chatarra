...
function Cargo_Nodescargo({ClaUbicacionOrigen,cambionodesc,setcambionodesc,setPlanCarga,pesajeparcial,placadato,editBoxValue,setBoxPlanCarga,NomMotivoEntrada,setActualizar,row,setrow,Todos,setTodos,TodosChange,setTodosChange,setValidaCargo,Nocargo,setNocargo}) {
    ...
    if(NomMotivoEntrada===9 && (row || row==='')){
      const data11 = 
      [ `@pnClaUbicacion=${editBoxValue}${config.Separador}@pnIdBoleta=${placadato[0].IdBoleta}${config.Separador}@psObservaciones='${(placadato[0].Observaciones ? placadato[0].Observaciones.replace('#', '%23').replace(/\r\n|\n/gi,'').replace(/['\']/g,''): '')}'${config.Separador}@pnEsRevisionEfectuada=${placadato[0].EsRevisionEfectuada}${config.Separador}@pnClaTipoClasificacion=${placadato[0].ClaTipoClasificacion}${config.Separador}@pnEsNoCargoDescargoMaterial=${Todos}${config.Separador}@psNombrePcMod='${ipadress}'${config.Separador}@pnClaUsuarioMod=${NumbUsuario}${config.Separador}@psOrigen='WEB'`];
  ...
  if(NomMotivoEntrada===110 && row){
    const data72 = {
      parameters:
        ...
        (placadato[0].Observaciones ? placadato[0].Observaciones.replace('#', '%23').replace(/\r\n|\n/gi,'').replace(/['\']/g,''):'') +
